#Homework Number: Homework 2 Problem 1
#Name: Tycho Halpern
#ECN Login: thalper 
...
def get_encryption_key():
    with open (sys.argv[3], 'r') as keyFile:
        key = BitVector(textstring = keyFile.readline())
...
def encrypt():
    ...
    outFile.write(outBV.get_bitvector_in_hex())
    outFile.close()
            
def decrypt():
    key = get_encryption_key()
    round_keys = generate_round_keys(key)
    hexBV = open(sys.argv[2], 'r')
    ...
    outFile.write(outBV.get_bitvector_in_ascii().rstrip("\0"))
    outFile.close()
...
if __name__ == "__main__":
    if sys.argv[1] == "-e":
        ...
        print("neither encryption or decryption")
